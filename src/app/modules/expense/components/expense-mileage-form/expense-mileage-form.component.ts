...
export class ExpenseMileageFormComponent implements OnInit, AfterViewInit {
  ...
  onSubmit() {
    if (this.form.valid) {
      if (!this.vehiclesService.isCustomizable(this.commute)) {
        this.commute.allowance =
          this.vehiclesService.vehicles[this.commute.vehicleSelected].allowance;
      }
      this.submitted = true;
      this.commute.mileageAllowance =
        this.commute.distance * this.commute.allowance;
