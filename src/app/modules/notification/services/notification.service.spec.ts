...
describe('NotificationService', () => {
	...
	describe('push()', () => {
		...
		it('should merge notification options with default options', () => {
			service.push("I'm a notification with custom options", 'default', {
				isSelfClosing: false,
				isDismissable: false,
			});
		...
		it('should launch a timer for selfclosing notifications only and remove it after the delay', () => {
			...
			service.push("I'm NOT a selfclosing notification", 'default', {
				isSelfClosing: false,
			});
			expect(service.startTimer).not.toHaveBeenCalled();

			service.push("I'm a selfclosing notification", 'default', {
				isSelfClosing: true,
				duration: 3,
			});
			...
			setTimeout(() => {
				expect(service.notifications.length).toEqual(1);
				expect(service.notifications[0].message).toEqual(
					"I'm a selfclosing notification"
				);
