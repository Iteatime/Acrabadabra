...
import { Timesheet } from "../../shared/models";
...
export class CalendarService {
  ...
  getWorkedTime(timesheet: Timesheet): WorkingEvent[] {
    ...
    if (Object.keys(timesheet.workingDays).length > 0) {
      const workedTimeObj = {};
      timesheet.workingDays[Object.keys(timesheet.workingDays)[0]].forEach(
        (event: WorkingEvent) => {
          if (event.unit) {
            const entry = workedTimeObj[event.unit];
            entry
              ? (workedTimeObj[event.unit] += event.time)
              : (workedTimeObj[event.unit] = event.time);
          }
        }
      );
      Object.keys(workedTimeObj).forEach((key) =>
        workedTime.push({
          time:
            key === "hours"
              ? Math.round(workedTimeObj[key])
              : workedTimeObj[key], // prevent 24 hours issue
          unit: key,
        })
      );
    }
  ...
  getWorkingDays(events: CalendarEvent[]): any {
    if (events[0] !== undefined) {
      ...
      for (
        let date = 0;
        date < new Date(lastDayOfMonth(events[0].start)).getDate();
        date++
      ) {
  ...
  getDayFromEvent(event: CalendarEvent<any>): WorkingEvent {
    return {
      time:
        ...
        this.timeUnits[event.title].timeInMinutes,
      unit: event.title,
    };
  }
  ...
  getNewEndDate(event: CalendarEvent<any>, time: number): Date {
    let date = addMinutes(
      event.start,
      time * this.timeUnits[event.title].timeInMinutes
    );
  ...
  public getFirstWorkingDay(timesheet: Timesheet): Date | boolean {
    ...
    if (Object.keys(timesheet.workingDays).length > 0) {
      ...
      timesheet.workingDays[timesheetDate].some(
        (event: WorkingEvent, day: number) => {
          date = new Date(
  ...
  public getLastWorkingDay(timesheet: Timesheet): Date | boolean {
    ...
    if (Object.keys(timesheet.workingDays).length > 0) {
      const timesheetDate = Object.keys(timesheet.workingDays)[0];
      [...timesheet.workingDays[timesheetDate]]
        .reverse()
        .some((event: WorkingEvent, day: number) => {
          date = new Date(
            +Number.parseInt(timesheetDate.split(".")[1], 10),
            +Number.parseInt(timesheetDate.split(".")[0], 10)
          );
