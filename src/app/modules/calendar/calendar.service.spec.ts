...
describe('CalendarService', () => {
	...
	beforeEach(() => {
		...
		calendarEvents = [
			{
				title: '',
				start: new Date(1900, 0, 1, 8),
				end: new Date(1900, 0, 1, 12),
			},
			{
				title: '',
				start: new Date(1900, 0, 2, 8),
				end: new Date(1900, 0, 2, 16),
			},
		];
	});
	...
	describe('getWorkingDays', () => {
		it('should return an array of the working days using the given calendar data', () => {
			expect(service.getWorkingDays(calendarEvents)).toEqual(
				testTimesheet.workingDays
			);
		});
	...
	describe('getFirstWorkingDay()', () => {
		it('should return the first day worked for this timesheet', () => {
			expect(service.getFirstWorkingDay(testTimesheet)).toEqual(
				new Date(1900, 0, 1)
			);
		});
	...
	describe('getLastWorkingDay()', () => {
		it('should return the last day worked for this timesheet', () => {
			expect(service.getLastWorkingDay(testTimesheet)).toEqual(
				new Date(1900, 0, 2)
			);
		});
