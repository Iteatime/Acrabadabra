...
export class InvoicePDFComponent {
  ...
  public async loadTimesheet() {
    ...
    this.expenseMileageTotal = this.timesheetService.getTotalAllowance();
    this.expenseMiscellaneousTotal =
      this.timesheetService.getTotalMiscellaneous();
    this.expenseFlatFeeTotal = this.timesheetService.getTotalFlatFee();
  ...
  private _sumCalcul(): void {
    ...
    this.workedTime.forEach((entry) => {
      this.performanceTotal +=
        entry.time * this.timesheetService.timesheet.invoice.workedRate;
    });
