...
export class BillingFormComponent implements OnInit {
  ...
  ngOnInit() {
    this.billingForm = this.registerFormControls();
    this.billingDataService.onDataChanged.subscribe((bill) => {
      this.bill = bill;
      this.updateForm();
    });
  }
  ...
  updateForm(): void {
    Object.keys(this.controlsBindings()).forEach(control => {
      const controlBinding = this.controlsBindings()[control];
      this[control].setValue(controlBinding.parent[controlBinding.value]);
    });
  }
